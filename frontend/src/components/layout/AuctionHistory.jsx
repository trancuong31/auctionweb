...
const AuctionHistory = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    axiosClient
      ...
      .catch((err) => setError("Lỗi khi tải dữ liệu lịch sử đấu giá", + (err.response.data.detail || "")))
  ...
  return (
    (visible || closing) && (
      <div
        className={
          "fixed inset-0 flex items-center justify-center z-50 max-sm:pt-[60px]  " +
          (closing ? "overlay-fade-out" : "overlay-fade-in")
        }
      >
        <div
          className={
            "mt-[220px] sm:mt-[60px] md:mt-[55px] bg-white rounded-xl shadow-2xl 2xl:mb-[10px] w-full max-w-lg sm:max-w-2xl md:max-w-4xl lg:max-w-7xl max-h-[95vh] overflow-hidden mx-2 sm:mx-4 md:mx-auto flex flex-col " +
            (closing ? "fade-slide-out" : "fade-slide-in")
          }
